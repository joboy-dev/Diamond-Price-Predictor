...
def predict_price(carat, depth, table, x_length, y_length, z_length, cut, color, clarity):
    ...
    cut_encoded = cut_mapping.get(cut, [0, 0, 0, 0, 0])
    color_encoded = color_mapping.get(color, [0, 0, 0, 0, 0, 0, 0])
    clarity_encoded = clarity_mapping.get(clarity, [0, 0, 0, 0, 0, 0, 0])
    
    # Create input array
    X_test = np.array([[carat, depth, table, x_length, y_length, z_length] + cut_encoded + color_encoded + clarity_encoded])
